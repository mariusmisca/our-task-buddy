...
export const CampaignHeader = ({ campaign, totalProposed, totalDelivered }: CampaignHeaderProps) => {
  const completionRate = campaign.totalLocations > 0 ? (campaign.completedLocations / campaign.totalLocations) * 100 : 0;
  const conversionRate = totalProposed > 0 ? (totalDelivered / totalProposed) * 100 : 0;
    ...
    <div className="space-y-4 mb-6">
      <Card className="p-4 bg-gradient-to-r from-primary/5 to-primary-glow/5 border-primary/20">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-xl font-bold text-foreground">Seguimiento de Acciones</h2>
            <p className="text-sm text-muted-foreground">{campaign.totalLocations} ubicaciones • 15 rutas • 3 vueltas</p>
          </div>
          <Badge variant="secondary" className="bg-primary/10 text-primary">
            <Calendar className="h-3 w-3 mr-1" />
            Ronda {campaign.currentRound} - Semana {campaign.currentWeek}
          </Badge>
        </div>
        <div className="mb-3">
          <div className="flex justify-between text-sm mb-1">
            <span className="text-muted-foreground">Progreso general</span>
            <span className="font-medium">{campaign.completedLocations}/{campaign.totalLocations}</span>
          </div>
