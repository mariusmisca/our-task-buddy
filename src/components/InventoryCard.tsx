...
export const InventoryCard = ({ action, inventory }: InventoryCardProps) => {
  const totalUsed = inventory.proposed + inventory.delivered;
  const usagePercentage = (totalUsed / action.stock) * 100;
      ...
      <div className="space-y-3">
        <div>
          <div className="flex justify-between text-sm mb-1">
            ...
            <span className="font-medium">{totalUsed}/{action.stock}</span>
          </div>
          <Progress 
            value={usagePercentage} 
            className="h-2"
            style={{ backgroundColor: `${action.hexColor}20` }}
          />
        </div>
